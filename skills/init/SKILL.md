---
name: init
description: One-time project bootstrap for lean-spec — scaffolds .lean-spec/rules.toml, .lean-spec/, .lean-spec/features/, and a multi-ecosystem .gitignore. Fail-loud preflight; idempotent (safe to re-run).
---
...
1. **Preflight (fail-loud).** Run `bin/lean-spec status` from the lean-spec runtime
...
2. **Scaffold `.lean-spec/rules.toml`** — if it does not already exist,
   copy `examples/rules.toml` from the lean-spec runtime into `.lean-spec/rules.toml`
   at the project root. If it already exists, leave it untouched (report
   that it already exists).

3. **Scaffold `.lean-spec/`** — if `.lean-spec/PRD.md` or `.lean-spec/CONSTITUTION.md` do
   not exist, copy the matching file from `templates/` into `.lean-spec/`. Do
   not overwrite either file if it already exists — `plan` (or
   the user) fills these in next.

4. **Scaffold `.lean-spec/features/`** — create the empty `.lean-spec/features/` directory if
   absent (features are created individually by `spec` via
...
5. **`.gitignore`** — ensure the project ignores build artifacts,
   ...
   lean-spec runtime's `examples/gitignore` into the project's `.gitignore`: create
