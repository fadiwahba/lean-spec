...
| R18 | Enforcement hardening from a from-scratch deep review (v1.3.0): the three hooks reject valid-but-non-object JSON instead of crashing (was a fail-open bypass); the `workflow.json` write guard normalizes + case-folds paths (`Workflow.json`/`../` no longer dodge it); corrupt `rules.toml`/`workflow.json` fail loudly, not with tracebacks; `--no-tdd` persists per-feature in `workflow.json` so the gate honors the opt-out; `reviewing→implementing` is CLI-gated on a `NEEDS_FIXES` verdict; `advance` is serialized with an `fcntl` lock and preserves file mode | the "discipline enforced by the harness, never prompt obedience" premise only holds if the harness itself can't be crashed into failing open or bypassed — the review found several such holes; closing them keeps the enforcement guarantees real under corrupt/adversarial state |
