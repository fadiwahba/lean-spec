...
case "$PROMPT_LOWER" in
  ...
  *"/lean-spec:review-spec "*|"/lean-spec:review-spec")
    COMMAND_HINT="This is the review phase. It must run in the Gemini Pro session. If you are not in the intended Pro session, stop and tell the human to rerun it there. Use the Architect role, update review.md, reconcile spec.md, use context7 when library or framework behavior matters, use sequential_thinking for multi-step or risky review work, use playwright for frontend/UI review before reporting the review complete unless it is unavailable, close any opened Playwright browser, context, or page before ending the phase, never save Playwright screenshots into the project root, save any captures only under lean-spec/features/<slug>/artifacts, and stop any local dev server or validation port you started before ending the phase."
    ;;
...
base = (
    ...
    "Store any screenshots, images, audio, PDFs, or other lean-spec evidence files only under lean-spec/features/<slug>/artifacts. "
