...
@test "advance preserves an existing workflow.json's file mode (does not downgrade to 600)" {
  ...
  chmod 644 .lean-spec/features/demo/workflow.json
  lean_spec advance demo specifying implementing
  [ "$status" -eq 0 ]
  run mode_of .lean-spec/features/demo/workflow.json
  [ "$output" = "644" ]
}

@test "ensure creates a new workflow.json per umask, not owner-only 600" {
  run bash -c "umask 022; '${LEAN_SPEC_BIN}' ensure demo >/dev/null && python3 -c \"import os; print(oct(os.stat('.lean-spec/features/demo/workflow.json').st_mode & 0o777)[2:])\""
  [ "$status" -eq 0 ]
  [ "$output" = "644" ]
}
